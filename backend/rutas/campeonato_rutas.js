  ...
  form.parse(req, function (err, fields, files) {
    ...
    camp.save(function (err) {
      if (err) {
        ...
        res.send({
          status: "Error",
          msg: "Los campeonatos no pudieron ser guardados",
        });
        return false;
      }
      res.send({
        status: "Ok",
        msg: "Los campeonatos fueron guardados satisfactoriamente",
      });
    });
  });
});
...
campeonato_rutas.get("/listarcamp", async function (req, res) {
  ...
  } else res.status(200).send({ status: "Ok", msg: "Campeonatos encontrados", campeonatos });
});
...
campeonato_rutas.post("/eliminarcamp", async function (req, res) {
  ...
  campeonatomodel.deleteOne({ nombrecamp }, function (error, camp) {
    ...
    if (error) {
      res.send({
        status: "Error",
        msg: "El campeonato NO fue encontrado. Error",
      });
      return false;
    } else {
      if (camp) {
        res.send({
          status: "Ok",
          msg: "El campeonato fue eliminado satisfactoriamente",
          dato: camp,
        });
      } else {
        res.send({
          status: "Error",
          msg: "El campeonato No pudo ser eliminado",
        });
      }
    }
  });
  ...
  campeonatomodel.updateOne(filter, updateCamp, function (error, camp) {
    ...
    if (error) {
      ...
      if (camp) {
        res.send({
          status: "Ok",
          msg: "El campeonato fue editado satisfactoriamente",
          dato: camp,
        });
      } else {
        res.send({
          status: "Error",
          msg: "El campeonato No pudo ser editado, porque no fue encontrado",
        });
      }
    }
  });
