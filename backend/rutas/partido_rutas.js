...
partido_rutas.post("/guardar", function (req, res) {
  ...
  camp.save(function (err) {
    if (err) {
      res.send({
        status: "Error",
        msg: "Los partidos no pudieron ser guardados",
      });
      return false;
    }
    res.send({
      status: "Ok",
      msg: "Los partidos fueron guardados satisfactoriamente",
    });
  });
});
...
partido_rutas.get("/listarpartidos", async function(req,res){
  ...
  if (partidos == null) {
    res
      .status(400)
      .send({ status: "Error", msg: "La base de datos está vacía", partidos });
  } else res.status(200).send({ status: "Ok", msg: "Partidos encontrados", partidos });
});
  ...
  partidosmodel.deleteOne({ encuentro }, function (error, camp) {
    ...
    if (error) {
      ...
      if (camp) {
        res.send({
          status: "Ok",
          msg: "El partido fue eliminado satisfactoriamente",
          dato: camp,
        });
      } else {
...
partido_rutas.post("/editar", function (req, res) {
  ...
  partidosmodel.updateOne(filter, updatePartido, function (error, part) {
    console.log(part);
    if (error) {
      res.send({ status: "Error", msg: "El partido NO fue encontrado. Error" });
      return false;
      ...
      if (part) {
        res.send({
          status: "Ok",
          msg: "El partido fue editado satisfactoriamente",
          dato: part,
        });
      } else {
        res.send({
          status: "Error",
          msg: "El partido No pudo ser editado, porque no fue encontrado",
        });
      }
    }
  });
