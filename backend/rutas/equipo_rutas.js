...
equipo_rutas.post("/guardar", function (req, res) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    eq.save(function (err) {
      ...
      res.send({
        status: "Ok",
        msg: "El equipo fue guardado satisfactoriamente",
      });
    });
  });
});
...
equipo_rutas.get("/listar", async function (req, res) {
  ...
  if (equipos == null) {
    res
      .status(400)
      .send({ status: "Error", msg: "La base de datos está vacía", equipos });
  } else res.status(200).send({ status: "Ok", msg: "Jugadores encontrados", equipos });
...
equipo_rutas.post("/eliminar", async function (req, res) {
  ...
  equipomodel.deleteOne({ nombre }, function (error, equ) {
    ...
    if (error) {
      res.send({
        status: "Error",
        msg: "El equipo NO fue encontrado por Error",
      });
      return false;
    } else {
      if (equ) {
        res.send({
          status: "Ok",
          msg: "El equipo fue eliminado satisfactoriamente",
          dato: equ,
        });
      } else {
  ...
  equipomodel.updateOne(filter, updateDoc, function (error, eq) {
    ...
    if (error) {
      ...
      if (eq) {
        res.send({
          status: "Ok",
          msg: "El equipo fue inscrito satisfactoriamente",
          dato: eq,
        });
      } else {
        res.send({
          status: "Error",
          msg: "El equipo No pudo ser inscrito, porque no fue encontrado",
        });
      }
    }
  });
  ...
  equipomodel.updateOne(filter, updateDoc, function (error, eq) {
    ...
    if (error) {
      ...
      if (eq) {
        res.send({
          status: "Ok",
          msg: "El equipo fue aprobado satisfactoriamente",
          dato: eq,
        });
      } else {
        res.send({
          status: "Error",
          msg: "El equipo No pudo ser aprobado, porque no fue encontrado",
        });
      }
    }
  });
