...
posicion_rutas.post("/posiciones", function(req,res){
    ...
    camp.save(function(err){
        if(err){
            res.send({status:"Error",msg:"Las posiciones no pudieron ser guardados"})
            return false;
        }
        res.send({status:"Ok",msg:"Las posiciones fueron guardadas satisfactoriamente"})
    })
});
...
posicion_rutas.get("/listarposiciones", async function(req,res){
    ...
    console.log(posiciones);
        if (posiciones == null){
            res.status(400).send({status:"Error", msg:"La base de datos está vacía", posiciones});
        }else
...
posicion_rutas.post("/eliminarposicion", async function(req,res){
    ...
    posicionmodel.deleteOne({nombre},function(error,camp){
        console.log(camp);
        if (error){
            res.send({status:"Error",msg:"La Posición NO fue encontrada. Error"});
            return false;
        }
        else{
            if (camp){
                res.send({status:"Ok",msg:"La Posición fue eliminada satisfactoriamente", dato:camp})
            }
            else{
...
posicion_rutas.post("/editarposicion", function(req,res){
    ...
    posicionmodel.updateOne(filter, updatePosicion, function(error,jug){
        console.log(camp);
        if (error){
            res.send({status:"Error",msg:"La Posicion NO fue encontrada. Error"});
            return false;
        }
        else{
            if (camp){
                res.send({status:"Ok",msg:"La Posicion fue editads satisfactoriamente", dato:camp})
            }
            else{
                res.send({status:"Error",msg:"La Posicion No pudo ser editada, porque no fue encontrada"})
            }
        }
    })
});
